#!/usr/bin/python3
"""
This is a Fabric script that creates, distributes an archive to my webservers
...
def do_clean(number=0):
    ...
    """
    archives_list = sorted(os.listdir('versions/'), reverse=True)

    starting_idx = int(number)

    if not starting_idx:
        starting_idx += 1

    if starting_idx < len(archives_list):
        archives_list = archives_list[starting_idx:]
    else:
        archives_list = []

    for elemnt in archives_list:
        os.unlink('versions/{}'.format(elemnt))
    ...
    cmd_str = "".join(cmd)
    cmd_str = cmd_str.format(starting_idx + 1)
